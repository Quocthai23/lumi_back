...
public class OptionSelectServiceImpl implements OptionSelectService {
    ...
    public OptionSelectDTO create(OptionSelectDTO dto) {
        if (repo.existsByOptionGroup_IdAndCodeIgnoreCase(dto.getOptionGroupId(), dto.getCode())) throw new IllegalArgumentException(
            "OptionSelect code duplicated in group"
        );
        OptionSelect e = mapper.toEntity(dto);
        // Load OptionGroup từ repository để đảm bảo entity tồn tại
        OptionGroup group = optionGroupRepo
            .findById(dto.getOptionGroupId())
    ...
    public OptionSelectDTO update(Long id, OptionSelectDTO dto) {
        ...
        if (
            !e.getCode().equalsIgnoreCase(dto.getCode()) &&
    ...
    public void delete(Long id) {
        List<com.lumiere.app.domain.OptionVariant> optionVariants = optionVariantRepo.findByOptionSelect_IdIn(List.of(id));
